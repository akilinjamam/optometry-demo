...
export const fetchPostPaymentData = async (paymentDataContainer) => {
    try {
        const response = await axios.post(`http://localhost:5000/api/v1/payment/create-payment`, paymentDataContainer);
...
export const fetchGetPaymentData = async (cus_email, setErrorHolder) => {
    try {
        const response = await axios.get(`http://localhost:5000/api/v1/payment?cus_email=${cus_email}`, {
            headers: {
                authorization: `Bearer ${localStorage.getItem('oab-access-token')}`
            }
        });
